...
def compute_cost(A2, Y, parameters):
    ...
    #print((np.multiply(np.log(A2),Y)).shape)
    #print((np.multiply(np.log(1-A2),1-Y)).shape)
    
    # Compute the cross-entropy cost
    ### START CODE HERE ### (≈ 2 lines of code)
    logprobs = np.multiply(np.log(A2).ravel(),Y.ravel()) + np.multiply(np.log(1-A2).ravel(),(1-Y).ravel())
    ### END CODE HERE ###
